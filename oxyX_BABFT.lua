...
local function objToRobloxStudioScript(vertices, faces, scale, objName)
    ...
    table.insert(lines, "-- Generated by oxyX BABFT Suite v2.3 | Powered by oxyX Market")
...
local infoLines = {
    {"⚡ oxyX BABFT Suite v2.3", Color3.fromRGB(200, 150, 255), 16},
...
local function setupKeybind()
    ...
    statusLbl.Position = UDim2.new(0, 0, 1, -16)
    statusLbl.BackgroundTransparency = 1
    statusLbl.Text = "⌨️ Toggle: RightShift | Click X to close"
...
local function setupChatCommand()
    ...
    local chatBar = player:FindFirstChild("PlayerGui") and player.PlayerGui:FindFirstChild("Chat")
    if not chatBar then
        -- Try alternative path
        pcall(function()
            chatBar = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("Chat", 5)
        end)
    end
    
    -- Simple chat command detection via messages
    local function onChatted(message)
        if message:lower() == "/oxyx" or message:lower() == "/babft" or message:lower() == "/oxyx show" or message:lower() == "/oxyx hide" then
            local show = message:lower():find("show") or (message:lower() == "/oxyx" or message:lower() == "/babft")
            
            if show then
                ScreenGui.Enabled = true
                MainFrame.Visible = true
                tween(MainFrame, {Size = UDim2.new(0, 560, 0, 520)}, 0.3)
                notify("oxyX BABFT", "GUI Opened! Use RightShift to toggle.", 3)
            else
                tween(MainFrame, {Size = UDim2.new(0, 0, 0, 0)}, 0.3)
                task.delay(0.35, function()
                    ScreenGui.Enabled = false
...
task.delay(0.6, function()
    notify("oxyX BABFT Suite v2.3", "Loaded! Executor: " .. executor, 4)
end)

-- ============================================================
-- END OF SCRIPT
-- ============================================================
-- oxyX BABFT Suite v2.3
